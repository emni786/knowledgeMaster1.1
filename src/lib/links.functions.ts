...
async function aiAnalyze(input: {
  ...
  const system =
    ...
    "- Bangla style — CRITICAL: write the way an educated Bangla-speaking friend would naturally describe the page to another Bangla reader, NOT a literal word-for-word translation. The Bangla must sound like original conversational Bangla prose, not stilted machine translation. Use ordinary spoken-but-literate Bangla (not heavy সাধু ভাষা / news-anchor formal register). Verbs and connectors should flow naturally — pair the English noun with the Bangla particle/verb a Bangla speaker would actually use. Avoid robotic filler openings like 'এই নিবন্ধটি …', 'এই এন্ট্রিটি …', 'এই লিঙ্কটি বোঝায় …', 'এই পোস্টটি আলোচনা করে …' — open by describing the content itself, the same way you would in spoken Bangla. Reorder sentence structure where English-to-Bangla word-order requires it; never preserve English syntax just to stay close to the source.\n" +
    "- In any Bangla output keep technical / proper nouns in English exactly (React, API, GitHub, OpenAI, LLM, machine learning, framework, dataset, Claude, GPT, repo, dataset, post, video, link, etc.). Do NOT transliterate them. But surround them with natural Bangla particles and verbs ('GitHub-এ একটা repo', 'API call করার সময়', 'এই video-টায় দেখানো হয়েছে').\n" +
